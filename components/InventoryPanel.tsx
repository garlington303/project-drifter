...
export const InventoryPanel: React.FC<InventoryPanelProps> = ({ inventory, equipment, onUpdate }) => {
  ...
  const handleDragStart = (e: React.DragEvent, source: DragSource) => {
    ...
    // Fix for "dragging entire element": use just the icon as the drag image
    const target = e.currentTarget as HTMLElement;
    const iconElement = target.querySelector('.item-icon');
    
    if (iconElement) {
       e.dataTransfer.setDragImage(iconElement, 20, 20);
    }
  };
  ...
  const handleInventoryRightClick = (e: React.MouseEvent, index: number) => {
    ...
    if (item && item.slot) {
        ...
        if (unequippedItem) {
            // Put the old item in the spot we just cleared
  ...
  const handleEquipRightClick = (e: React.MouseEvent, slot: EquipmentSlot) => {
    ...
    if (unequippedItem) {
        ...
        if (!added) {
            // Inventory full, cancel action (put item back)
  ...
  const getRarityColor = (rarity: Rarity) => {
    switch (rarity) {
      case Rarity.Common: return 'border-neutral-600';
      case Rarity.Uncommon: return 'border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]';
      case Rarity.Rare: return 'border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.3)]';
      case Rarity.Epic: return 'border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.3)]';
      case Rarity.Legendary: return 'border-amber-400 shadow-[0_0_10px_rgba(251,191,36,0.5)]';
  ...
  const RenderEquipSlot = ({ slot, icon: Icon }: { slot: EquipmentSlot, icon: any }) => {
    ...
    return (
      <div 
        className={`
            w-16 h-16 bg-neutral-900 border rounded flex items-center justify-center relative transition-colors 
            ${item ? `${getRarityColor(item.rarity)} cursor-grab active:cursor-grabbing` : 'border-neutral-700 hover:border-neutral-500 cursor-default'}
        `}
        draggable={!!item}
        ...
        title={item ? `${item.name} (Right-click to unequip)` : slot}
      >
        {item ? (
  ...
  return (
    <div className="flex h-full gap-6 select-none">
      ...
      <div className="w-1/3 flex flex-col items-center bg-neutral-900/30 border border-neutral-800 rounded-lg p-4 relative">
      ...
      <div className="w-2/3 flex flex-col h-full bg-neutral-900/30 border border-neutral-800 rounded-lg overflow-hidden">
          ...
          <div className="flex-1 p-4">
            <div className="grid grid-cols-5 gap-3">
                ...
                <div
                    key={index}
                    className={`
                    relative aspect-square bg-neutral-900 rounded border-2 
                    ${item ? `${getRarityColor(item.rarity)} cursor-grab active:cursor-grabbing` : 'border-neutral-800 cursor-default'}
                ...
                >
                    {item && (
                    <>
                        {/* We add 'item-icon' class here to reference it in setDragImage */}
                        <div className="text-3xl select-none drop-shadow-lg pointer-events-none item-icon">{item.icon}</div>
                        {item.quantity > 1 && (
                        ...
                        {/* Tooltip */}
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-56 bg-neutral-950 border border-neutral-800 rounded-lg p-3 z-50 hidden group-hover:block shadow-xl pointer-events-none">
          ...
          <div className="bg-neutral-950 border-t border-neutral-800 p-2 px-4 flex justify-between items-center text-xs font-mono text-neutral-500">
